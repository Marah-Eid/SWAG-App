...
const VerifyVendorEmailScreen = () => {
    const router = useRouter();
    const params = useLocalSearchParams();
    const { emailOrPhone, fullName, shopName, shopType, password, categories } = params;
    ...
    const handleVerify = async () => {
        ...
        setLoading(true);
        const result = await otpAPI.verifyOtp(emailOrPhone, fullCode, 'signup');
        setLoading(false);
        ...
        // Navigate to location screen passing all accumulated data
        router.push({
            pathname: '/auth/LocationVendor',
            params: { emailOrPhone, fullName, shopName, shopType, password, categories },
        });
    };

    const handleResend = async () => {
        if (!canResend) return;
        const result = await otpAPI.sendOtp(emailOrPhone, 'signup');
        if (result.success) {
    ...
    return (
        ...
        >
            ...
            <View style={styles.topSection}>
                <TouchableOpacity
                    onPress={() => router.push('/auth/SignUpVendor')}
            ...
            <View style={styles.whiteCard}>
                <View style={styles.infoContainer}>
                    ...
                    <Text style={styles.emailText}>{emailOrPhone || 'vendor@example.com'}</Text>
                </View>
...
const styles = StyleSheet.create({
    ...
    headerSubtitle: { fontSize: 14, color: '#2D3E5E', textAlign: 'center', opacity: 0.8, lineHeight: 22, fontWeight: '500' },
