...
const VerifySignupScreen = () => {
    ...
    const { emailOrPhone, fullName, city, password } = useLocalSearchParams();
    ...
    const handleVerify = async () => {
        ...
        // Step 1: Verify OTP
        const otpResult = await otpAPI.verifyOtp(emailOrPhone, fullCode, 'signup');
        if (!otpResult.success) {
            setLoading(false);
            showAlert('Error', otpResult.error);
            return;
        }
        ...
        if (!registerResult.success) {
            setLoading(false);
            showAlert('Error', registerResult.error);
            return;
        }

        // Step 3: Dispatch login to set auth state (token already in storage from register)
        dispatch(loginUser({ emailOrPhone, password, role: 'Customer' }));
        setLoading(false);

        showAlert('Approved', 'Your account has been created!', () => {
            router.replace('/customer/Interests');
        });
    };
