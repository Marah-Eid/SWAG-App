...
// Make sure you have a CommentsModal for customers. 
...
const CustomerPosts = ({
  ...
  mediaWidth, mediaHeight, onVendorPress
}) => {
  ...
  // --- NEW: External Share Logic ---
  const handleShare = async () => {
    try {
      const result = await Share.share({
        title: `Post by ${vendorName}`,
        message: `Check out this ${isEvent ? 'event' : 'post'} by ${vendorName} on our App!\n\n${description}`,
      });
  ...
  // --- Smart Description Rendering (Hashtags & Read More) ---
  ...
  // --- Calculate exact aspect ratio to fit media perfectly ---
  ...
  return (
    <View style={styles.card}>
      ...
      <View style={styles.footer}>
        ...
        {/* SHARE BUTTON: Triggers handleShare */}
